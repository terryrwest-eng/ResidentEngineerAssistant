...
1. **Model name to config** — `backend/app/core/config.py` (~line 29): add `GEMINI_MODEL = os.environ.get("GEMINI_MODEL", "gemini-3.6-flash")` (released 2026-07-21: audio/image/PDF input, structured output, toggleable thinking budgets, $1.50/$7.50 per M tokens — cheaper than 2.5 Pro and replaces it before its **2026-10-16 retirement**) plus `GEMINI_MODEL_HEAVY` (optional Pro-tier override for handwriting extraction if testing shows Flash falls short) and `GEMINI_MODEL_LITE` (classification). Replace the 4 hardcoded `"gemini-2.5-pro"` strings: `ai.py:75`, `schedule.py:53`, `pdf_search.py:38`, `dispatch_parser.py:28`. Set explicit thinking budgets per task type: ~0-2048 for transcription/classification, 8192+ for handwriting/dispatch extraction. **Acceptance test before committing to the default:** run one real handwritten timesheet + one real 5-min dictation through 3.6 Flash and compare against 2.5 Pro output.
