...
#
# The second one needs allow_duplicate, exactly as the UI does: a second report
# for a date that already has one is refused with 409 until the user answers the
# duplicate warning with "Keep both". A split shift is that answer, so the flow
# here mirrors the real one rather than bypassing the guard.
for i, (start, end, area) in enumerate([("06:30", "15:00", "Genesee - OHL - Paving"),
    ...
    client.post("/api/reports", json={
        ...
        "status": "draft",
        **({"allow_duplicate": True} if i else {}),
    })
